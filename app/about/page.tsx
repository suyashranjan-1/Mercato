 ...
 * This is essential for Next.js to avoid hydration errors caused by Math.random().
 */
function getParticles(count: number) {
  // Seeded pseudo-random generator (LCG) for deterministic values
  ...
  for (let i = 0; i < count; i++) {
    ...
    const animationDuration = 3 + random() * 4;
    particles.push({
      left: `${left}%`,
      top: `${top}%`,
...
export default function About() {
  ...
  // Generate particles ONCE for both SSR and CSR
  const particleCount = 20;
  const particles = getParticles(particleCount);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black text-white overflow-hidden">
      ...
      <section
        id="hero"
        className="relative min-h-screen flex items-center justify-center px-4 py-16 sm:py-20 lg:py-32"
      >
        ...
        {/* Floating particles (deterministic for SSR/CSR) */}
        ...
        <div
          className={`w-full max-w-7xl mx-auto text-center relative z-10 transition-all duration-1000 ${
      ...
      <section
        id="stats"
        className="relative py-12 sm:py-16 lg:py-20 px-4 bg-gradient-to-r from-slate-900/30 to-slate-800/20"
      >
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 text-center hover:border-slate-600/50 hover:bg-slate-800/50 transition-all duration-300 group transform ${
                  isInView("stats")
      ...
      <section id="philosophy" className="py-16 sm:py-24 lg:py-32 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div
            className={`relative mb-12 sm:mb-16 transform transition-all duration-1000 delay-300 ${
              isInView("philosophy")
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-10"
            }`}
              ...
              <div className="flex flex-wrap justify-center mb-6 sm:mb-8 gap-2 sm:gap-4">
                {["vision", "values", "approach"].map((tab) => (
                  <button
                    ...
                    className={`px-4 sm:px-6 lg:px-8 py-2 sm:py-3 lg:py-4 rounded-xl sm:rounded-2xl transition-all duration-300 font-semibold text-sm sm:text-base ${
                      activeTab === tab
                      ...
                      {values.map((value, index) => (
                        ...
                        >
                          <div className="flex items-center mb-3 sm:mb-4">
                            <div
                              className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-r ${value.color} flex items-center justify-center text-lg sm:text-xl mr-3 sm:mr-4 group-hover:scale-110 transition-transform duration-300`}
                            >
                              {value.icon}
                            </div>
                      ...
                      {[
                        {
                          ...
                          desc: "Deep dive into challenges and opportunities",
                          icon: "🔬",
                        },
                        {
                          title: "Design & Build",
                          desc: "Create scalable, intelligent solutions",
                          icon: "🛠️",
                        },
                        {
                          title: "Deploy & Optimize",
                          desc: "Deliver results and continuous improvement",
                          icon: "🚀",
                        },
                      ].map((step, index) => (
                        <div
                          key={index}
                          className="text-center p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 hover:border-slate-600/50 transition-all duration-300 group hover:scale-105"
                        >
      ...
      <section
        id="team"
        className="py-16 sm:py-24 lg:py-32 px-4 bg-gradient-to-br from-slate-900/50 to-slate-800/30"
      >
        <div className="max-w-7xl mx-auto">
          <div
            className={`text-center mb-12 sm:mb-16 lg:mb-20 transform transition-all duration-1000 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {teamMembers.map((member, index) => (
              <div
                key={member.id}
                className={`group relative overflow-hidden rounded-2xl sm:rounded-3xl bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 hover:border-slate-600/50 transition-all duration-500 hover:transform hover:scale-105 ${
                  isInView("team")
                    ? "animate-slideUp opacity-100"
                    : "opacity-0 translate-y-10"
                }`}
      ...
      <section id="join" className="py-16 sm:py-24 lg:py-32 px-4">
        <div className="max-w-7xl mx-auto">
          <div
            className={`relative rounded-2xl sm:rounded-3xl overflow-hidden transform transition-all duration-1000 ${
              isInView("join")
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-10"
            }`}
