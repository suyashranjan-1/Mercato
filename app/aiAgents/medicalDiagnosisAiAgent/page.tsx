...
import { Stethoscope, Clock, Users, Star, CheckCircle, ArrowRight, Bot, Shield, Zap, BarChart3, Globe, Play, ChevronRight, Brain, Cpu, Database, Settings, Mail, MessageSquare, Smartphone, HeartPulse, FileText, Search } from 'lucide-react';
import { NavbarDemo } from "@/components/navbar";
import Footer from "@/components/Footer";

export default function MedicalDiagnosisAIAgent() {
    ...
    const stats = [
        { label: 'Avg Diagnosis Time', value: '<30s', icon: Clock, color: 'from-blue-500 to-cyan-500' },
        { label: 'Symptom Database', value: '25M+', icon: Database, color: 'from-yellow-500 to-orange-500' },
        { label: 'Diagnosis Accuracy', value: '97.5%', icon: CheckCircle, color: 'from-emerald-500 to-teal-500' },
        { label: 'Supported Conditions', value: '10K+', icon: HeartPulse, color: 'from-purple-500 to-pink-500' },
    ];

    const features = [
        {
            title: 'Instant Symptom Analysis',
            description: 'Input patient symptoms and receive accurate, prioritized diagnoses within seconds.',
            icon: Stethoscope,
            gradient: 'from-blue-500 to-cyan-500'
        },
        {
            title: 'Personalized Recommendations',
            description: 'AI delivers tailored treatment suggestions, triage urgency, and next steps based on patient history.',
            icon: Brain,
            gradient: 'from-purple-500 to-pink-500'
        },
        {
            title: 'Multi-Modal Data Support',
            description: 'Integrate lab results, imaging, and EHR data for comprehensive diagnostic insights.',
            icon: Cpu,
            gradient: 'from-emerald-500 to-teal-500'
        },
        {
            title: 'Drug Interaction Alerts',
            description: 'Automatic detection and warning for harmful medication interactions and allergies.',
            icon: Shield,
            gradient: 'from-orange-500 to-red-500'
        },
        {
            title: 'Telemedicine Integration',
            description: 'Seamless handoff to telehealth providers for urgent or specialist care.',
            icon: Globe,
            gradient: 'from-indigo-500 to-purple-500'
        },
        {
            title: 'Continuous Learning Engine',
            description: 'Updates knowledge base with the latest peer-reviewed studies and clinical guidelines.',
            icon: Zap,
            gradient: 'from-pink-500 to-rose-500'
        },
        {
            title: 'Patient Monitoring',
            description: 'Track patient symptoms and progress over time for proactive intervention.',
            icon: Users,
            gradient: 'from-rose-500 to-pink-500'
        },
        {
            title: 'Regulatory Compliance',
            description: 'HIPAA/GDPR compliant with end-to-end encryption for all patient data.',
            icon: Shield,
    ...
    const howItWorks = [
        {
            step: 1,
            title: 'Symptom Input',
            description: 'Patient or clinician enters symptoms, medical history, and any test results.',
            icon: MessageSquare,
            color: 'from-blue-500 to-cyan-500'
        },
        {
            step: 2,
            title: 'AI Diagnostic Processing',
            description: 'AI compares input to millions of clinical cases, medical literature, and guidelines.',
            icon: Brain,
            color: 'from-purple-500 to-pink-500'
        },
        {
            step: 3,
            title: 'Diagnosis & Triage',
            description: 'Returns likely diagnoses, risk factors, and triage recommendations for follow-up.',
            icon: Stethoscope,
            color: 'from-emerald-500 to-teal-500'
        },
        {
            step: 4,
            title: 'Care Pathway & Monitoring',
            description: 'Suggests next steps, self-care, or specialist referral and tracks patient progress.',
            icon: HeartPulse,
    ...
    const integrations = [
        { name: 'Epic', category: 'EHR', logo: '🏥' },
        { name: 'Cerner', category: 'EHR', logo: '💊' },
        { name: 'Allscripts', category: 'EHR', logo: '📋' },
        { name: 'Apple Health', category: 'Patient Data', logo: '🍎' },
        { name: 'Google Fit', category: 'Patient Data', logo: '📱' },
        { name: 'Teladoc', category: 'Telemedicine', logo: '🩺' },
        { name: 'Zoom', category: 'Telehealth', logo: '📹' },
        { name: 'Slack', category: 'Team Communication', logo: '💬' },
        { name: 'Twilio', category: 'SMS', logo: '📲' },
        { name: 'FHIR', category: 'Health Data', logo: '🧬' },
        { name: 'MedlinePlus', category: 'Knowledge Base', logo: '📚' },
        { name: 'UpToDate', category: 'Clinical Guidance', logo: '📖' },
        { name: 'Redox', category: 'Healthcare API', logo: '🔗' },
        { name: 'Azure Health', category: 'Cloud', logo: '☁️' },
        { name: 'AWS HealthLake', category: 'Cloud', logo: '☁️' },
        { name: 'FHIR Cloud', category: 'Cloud', logo: '☁️' },
    ];

    const channels = [
        { name: 'Web Dashboard', icon: Smartphone, description: 'Access diagnostics from any device' },
        { name: 'Email Reports', icon: Mail, description: 'Share results securely with patients and providers' },
        { name: 'EHR Integration', icon: Database, description: 'Automatic record updates and sharing' },
        { name: 'Secure Messaging', icon: MessageSquare, description: 'Communicate and collaborate with care teams' }
    ];

    const benefits = [
        {
            title: 'Faster Diagnoses',
            description: 'Reduce diagnostic time from hours to seconds with instant AI suggestions.',
            percentage: '60x',
            metric: 'faster results'
        },
        {
            title: 'Improve Patient Outcomes',
            description: 'Earlier detection and intervention by recognizing subtle patterns and risk factors.',
            percentage: '38%',
            metric: 'outcome improvement'
        },
        {
            title: 'Reduce Diagnostic Errors',
            description: 'AI cross-checks rare and common conditions to minimize human oversight.',
            percentage: '72%',
            metric: 'error rate reduction'
        },
        {
            title: 'Lower Costs',
            description: 'Reduce unnecessary tests and hospital visits with accurate, early diagnosis.',
            percentage: '40%',
            metric: 'cost savings'
        },
        {
            title: 'Scalable Care Delivery',
            description: 'Handle unlimited patient inquiries simultaneously across multiple channels.',
            percentage: '∞',
            metric: 'scalability'
        }
    ];
            ...
            <section id="hero" className="relative min-h-screen flex items-center justify-center px-4 py-16 sm:py-20 lg:py-32">
                ...
                <div className={`w-full max-w-7xl mx-auto text-center relative z-10 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    ...
                    <div className="text-base sm:text-lg md:text-xl lg:text-2xl text-slate-400 max-w-4xl mx-auto leading-relaxed mb-8 sm:mb-12 px-4">
                        Empower clinicians and patients with instant, AI-driven diagnosis, triage, and care recommendations. Trusted by hospitals and telehealth providers worldwide.
                    </div>
                        ...
                        <div className="relative bg-slate-900/50 backdrop-blur-sm rounded-3xl p-6 sm:p-8 border border-slate-700/50 hover:border-slate-600/50 transition-all duration-500 group">
                            ...
                            <div className="flex items-center justify-between mb-6 p-4 bg-slate-800/40 rounded-2xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-semibold">Medical Diagnosis AI Agent</h3>
                                        <div className="text-slate-400 text-sm flex items-center gap-2">
                                            <div className="w-2 h-2 bg-emerald-400 rounded-full animate-ping"></div>
                                            Online • Diagnosing instantly
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right">
                                        <div className="text-sm font-medium text-emerald-400">97.5%</div>
                                        <div className="text-xs text-slate-400">Accuracy</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-sm font-medium text-blue-400">&lt;30s</div>
                                        <div className="text-xs text-slate-400">Avg Diagnosis</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="flex gap-3 animate-slideInLeft">
                                    ...
                                    <div className="bg-slate-800/50 rounded-2xl p-3 flex-1 max-w-xs">
                                        <span className="text-sm">My child has a fever, cough, and fatigue. What could it be?</span>
                                ...
                                <div className="flex gap-3 justify-end animate-slideInRight" style={{ animationDelay: '0.5s' }}>
                                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-3 max-w-sm">
                                        <span className="text-sm">Possible diagnoses: Viral infection (flu, COVID-19), strep throat, or common cold. Recommend monitoring temperature, hydration, and seeking medical review if symptoms worsen.</span>
                                ...
                                <div className="flex gap-3 animate-slideInLeft" style={{ animationDelay: '1s' }}>
                                    ...
                                    <div className="bg-slate-800/50 rounded-2xl p-3 flex-1 max-w-xs">
                                        <span className="text-sm">Are there any urgent warning signs I should watch for?</span>
                                ...
                                <div className="flex gap-3 animate-slideInLeft" style={{ animationDelay: '1s' }}>
                                    ...
                                    <div className="bg-slate-800/50 rounded-2xl p-3 flex-1 max-w-xs">
                                        <span className="text-sm">Seek immediate care if difficulty breathing, persistent high fever, or lethargy occurs.</span>
                                        <span className="text-xs text-slate-400 mt-1 block">10:13 AM</span>
                                    </div>
                                </div>
                            </div>
            ...
            <section id="features" className="py-16 sm:py-24 lg:py-32 px-4 bg-gradient-to-br from-slate-900/50 to-slate-800/30">
                <div className="max-w-7xl mx-auto">
                    <div className={`text-center mb-12 sm:mb-16 lg:mb-20 transform transition-all duration-1000 ${isInView('features') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        ...
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold max-w-5xl mx-auto mb-6 sm:mb-8 leading-tight px-4">
                            Comprehensive AI for
                            <span className="bg-gradient-to-r from-emerald-400 to-blue-600 bg-clip-text text-transparent"> medical diagnosis</span>
                        </h2>
                    </div>
            ...
            <section id="channels" className="py-16 sm:py-24 lg:py-32 px-4">
                <div className="max-w-7xl mx-auto">
                    <div className={`text-center mb-12 sm:mb-16 lg:mb-20 transform transition-all duration-1000 ${isInView('channels') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        ...
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold max-w-5xl mx-auto mb-6 sm:mb-8 leading-tight px-4">
                            Connected to your
                            <span className="bg-gradient-to-r from-blue-400 to-cyan-600 bg-clip-text text-transparent"> healthcare ecosystem</span>
                        </h2>
                    </div>
            ...
            <section id="capabilities" className="py-16 sm:py-24 lg:py-32 px-4">
                <div className="max-w-7xl mx-auto">
                    <div className={`text-center mb-12 sm:mb-16 lg:mb-20 transform transition-all duration-1000 ${isInView('capabilities') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        ...
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold max-w-5xl mx-auto mb-6 sm:mb-8 leading-tight px-4">
                            Smarter medical decisions with
                            <span className="bg-gradient-to-r from-purple-400 to-pink-600 bg-clip-text text-transparent"> advanced AI</span>
                        </h2>
                    </div>
                        ...
                        <div className={`relative transform transition-all duration-1000 ${isInView('capabilities') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
                            <div className="relative">
                                <div className="relative w-80 h-80 mx-auto">
                                    ...
                                    {[
                                        { label: 'Diagnosis', angle: 0, color: 'from-blue-500 to-cyan-500' },
                                        { label: 'Personalization', angle: 60, color: 'from-emerald-500 to-teal-500' },
                                        { label: 'Monitoring', angle: 120, color: 'from-orange-500 to-red-500' },
                                        { label: 'Compliance', angle: 180, color: 'from-purple-500 to-pink-500' },
                                        { label: 'Telehealth', angle: 240, color: 'from-yellow-500 to-orange-500' },
                                        { label: 'Knowledge', angle: 300, color: 'from-indigo-500 to-purple-500' }
                                    ].map((node, index) => {
                            ...
                            {[
                                {
                                    ...
                                    description: 'Rapidly assesses symptoms and medical history to suggest possible diagnoses and risk levels.',
                                    icon: Stethoscope,
                                    color: 'from-blue-500 to-cyan-500'
                                },
                                {
                                    title: 'Personalized Medical Guidance',
                                    description: 'Delivers tailored recommendations and care pathways based on patient-specific data.',
                                    icon: Brain,
                                    color: 'from-purple-500 to-pink-500'
                                },
                                {
                                    title: 'Continuous Patient Monitoring',
                                    description: 'Tracks patient status and notifies for early intervention in case of deterioration.',
                                    icon: Users,
                                    color: 'from-emerald-500 to-teal-500'
                                },
                                {
                                    title: 'Telehealth Ready',
                                    description: 'Integrates seamlessly with telemedicine platforms for remote care and referrals.',
                                    icon: Globe,
                                    color: 'from-orange-500 to-red-500'
                                },
                                {
                                    title: 'Clinical Knowledge Base',
                                    description: 'Draws on the latest studies, guidelines, and peer-reviewed evidence for every diagnosis.',
                                    icon: Database,
                                    color: 'from-indigo-500 to-purple-500'
                                },
                                {
                                    title: 'Privacy & Compliance',
                                    description: 'Meets HIPAA, GDPR, and local regulations with advanced encryption for all data.',
                                    icon: Shield,
            ...
            <section id="cta" className="py-16 sm:py-24 lg:py-32 px-4 bg-gradient-to-br from-slate-900/50 to-slate-800/30">
                <div className="max-w-4xl mx-auto text-center">
                    <div className={`transform transition-all duration-1000 ${isInView('cta') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        <div className="inline-flex items-center px-4 sm:px-6 py-2 mb-6 sm:mb-8 rounded-full border border-slate-700/50 bg-slate-800/30 backdrop-blur-sm text-xs sm:text-sm text-slate-300">
                            <Zap className="w-4 h-4 mr-2 text-emerald-400" />
                            Ready to elevate healthcare?
                        </div>
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-6 sm:mb-8 leading-tight">
                            Transform patient care with
                            <span className="bg-gradient-to-r from-emerald-400 to-blue-600 bg-clip-text text-transparent"> AI diagnosis</span>
                        </h2>
                        <p className="text-lg sm:text-xl text-slate-300 mb-8 sm:mb-12 leading-relaxed">
                            Join hospitals, clinics, and telehealth providers using AI to deliver faster, safer, and more accurate diagnoses. Get started with secure setup—no credit card required.
                        </p>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 pt-8 border-t border-slate-700/50">
                            {[
                                { label: 'Conditions Covered', value: '10K+' },
                                { label: 'Setup Time', value: '< 5 Days' },
                                { label: 'Patient Data Secure', value: '100%' },
                                { label: 'Clinical Integrations', value: '15+' }
                            ].map((item, index) => (
